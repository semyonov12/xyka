(() => {
    ...
    function rangeInit() {
        ...
        const priceDisplay = document.querySelector(".prices__value span");
        if (!buttons.length || !priceDisplay) return;
        const pricingData = {
            "2 кальяна": {
                3: 2e4,
                4: 23200,
        ...
        let currentHookahs = "2 кальяна";
        let currentHours = 3;
        buttons.forEach(button => {
            if (button.textContent.trim() === currentHookahs) button.classList.add("_active");
        });
        function formatNumberWithSpaces(number) {
            return number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
        }
        function updateTotalPrice() {
            const price = pricingData[currentHookahs][currentHours];
            if (price) priceDisplay.textContent = formatNumberWithSpaces(price);
        }
        ...
        nouislider.create(priceSlider, {
            ...
            tooltips: [ {
                to: function(value) {
                    ...
                    return `${rounded} ${getHoursWord(rounded)}`;
                }
            } ],
            pips: {
                mode: "steps",
        ...
        priceSlider.noUiSlider.on("update", function(values, handle) {
            currentHours = Math.round(values[handle]);
            updateTotalPrice();
        });
        buttons.forEach(button => {
            button.addEventListener("click", () => {
                ...
                currentHookahs = button.textContent.trim();
                updateTotalPrice();
            });
        });
        updateTotalPrice();
